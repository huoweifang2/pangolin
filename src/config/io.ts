...
import type { AgentShieldConfig, ConfigFileSnapshot, LegacyConfigIssue } from "./types.js";
...
function coerceConfig(value: unknown): AgentShieldConfig {
  if (!value || typeof value !== "object" || Array.isArray(value)) {
    return {};
  }
  return value as AgentShieldConfig;
}
...
function stampConfigVersion(cfg: AgentShieldConfig): AgentShieldConfig {
...
function warnIfConfigFromFuture(
  ...
  if (cmp < 0) {
    logger.warn(
      `Config was last written by a newer AgentShield (${touched}); current version is ${VERSION}.`,
    );
  }
}
...
function resolveConfigForRead(
  ...
  if (resolvedIncludes && typeof resolvedIncludes === "object" && "env" in resolvedIncludes) {
    applyConfigEnvVars(resolvedIncludes as AgentShieldConfig, env);
  }
...
export function createConfigIO(overrides: ConfigIoDeps = {}) {
  ...
  function loadConfig(): AgentShieldConfig {
    try {
      ...
      const preValidationDuplicates = findDuplicateAgentDirs(resolvedConfig as AgentShieldConfig, {
        env: deps.env,
  ...
  async function writeConfigFile(cfg: AgentShieldConfig, options: ConfigWriteOptions = {}) {
    clearConfigCache();
    ...
    const outputConfig =
      envRefMap && changedPaths
        ? (restoreEnvRefsFromMap(cfgToWrite, "", envRefMap, changedPaths) as AgentShieldConfig)
        : cfgToWrite;
    ...
    const auditRecordBase = {
      ...
      watchSession:
        typeof deps.env.AGENT_SHIELD_WATCH_SESSION === "string" &&
        deps.env.AGENT_SHIELD_WATCH_SESSION.trim().length > 0
          ? deps.env.AGENT_SHIELD_WATCH_SESSION.trim()
          : null,
      watchCommand:
        typeof deps.env.AGENT_SHIELD_WATCH_COMMAND === "string" &&
...
export function loadConfig(): AgentShieldConfig {
