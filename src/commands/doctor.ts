...
function resolveMode(cfg: AgentShieldConfig): "local" | "remote" {
...
export async function doctorCommand(
  ...
  intro("AgentShield doctor");

  const root = await resolveAgentShieldPackageRoot({
  ...
  let cfg: AgentShieldConfig = configResult.cfg;

  const configPath = configResult.path ?? CONFIG_PATH;
  if (!cfg.gateway?.mode) {
    const lines = [
      "gateway.mode is unset; gateway start will be blocked.",
      `Fix: run ${formatCliCommand("agent-shield configure")} and set Gateway mode (local/remote).`,
      `Or set directly: ${formatCliCommand("agent-shield config set gateway.mode local")}`,
    ];
    if (!fs.existsSync(configPath)) {
      lines.push(`Missing config: run ${formatCliCommand("agent-shield setup")} first.`);
    }
    note(lines.join("\n"), "Gateway");
  }
  ...
  if (shouldWriteConfig) {
    ...
    runtime.log(`Run "${formatCliCommand("agent-shield doctor --fix")}" to apply changes.`);
  }
