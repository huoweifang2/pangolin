...
function enhanceBrowserFetchError(url: string, err: unknown, timeoutMs: number): Error {
  ...
  if (looksLikeTimeout) {
    return new Error(
      `Can't reach the AgentShield browser control service (timed out after ${timeoutMs}ms). ${operatorHint} ${modelHint}`,
    );
  }
  return new Error(
    `Can't reach the AgentShield browser control service. ${operatorHint} ${modelHint} (${msg})`,
  );
}
