...
function ensureExperimentalWarningSuppressed(): boolean {
  ...
  if (isTruthyEnvValue(process.env.AGENT_SHIELD_NO_RESPAWN)) {
    return false;
  }
  if (isTruthyEnvValue(process.env.AGENT_SHIELD_NODE_OPTIONS_READY)) {
    return false;
  }
  ...
  process.env.AGENT_SHIELD_NODE_OPTIONS_READY = "1";
