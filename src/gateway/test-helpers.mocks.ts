...
export const setTestConfigRoot = (root: string) => {
  ...
  process.env.AGENT_SHIELD_CONFIG_PATH = path.join(root, "agent-shield.json");
};
...
vi.mock("../config/config.js", async () => {
  ...
  const resolveConfigPath = () => path.join(testConfigRoot.value, "agent-shield.json");
...
vi.mock("../plugins/loader.js", async () => {
  ...
  return {
    ...actual,
    loadAgentShieldPlugins: () => pluginRegistryState.registry,
  };
});
