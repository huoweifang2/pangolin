...
type EnsureAgentShieldPathOpts = {
  execPath?: string;
  cwd?: string;
  homeDir?: string;
...
function candidateBinDirs(opts: EnsureAgentShieldPathOpts): {
  prepend: string[];
  append: string[];
} {
  ...
  // Bundled macOS app: `agent-shield` lives next to the executable (process.execPath).
  try {
  ...
  const allowProjectLocalBin =
    ...
    isTruthyEnvValue(process.env.AGENT_SHIELD_ALLOW_PROJECT_LOCAL_BIN);
  if (allowProjectLocalBin) {
    const localBinDir = path.join(cwd, "node_modules", ".bin");
    if (isExecutable(path.join(localBinDir, "agent-shield"))) {
...
/**
 * Best-effort PATH bootstrap so skills that require the `agent-shield` CLI can run
 * under launchd/minimal environments (and inside the macOS app bundle).
 */
export function ensureAgentShieldCliOnPath(opts: EnsureAgentShieldPathOpts = {}) {
  if (isTruthyEnvValue(process.env.AGENT_SHIELD_PATH_BOOTSTRAPPED)) {
    return;
  }
  process.env.AGENT_SHIELD_PATH_BOOTSTRAPPED = "1";
