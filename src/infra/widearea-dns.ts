...
export function resolveWideAreaDiscoveryDomain(params?: {
  ...
  const candidate = params?.configDomain ?? env.AGENT_SHIELD_WIDE_AREA_DOMAIN ?? null;
...
function extractContentHash(zoneText: string): string | null {
  const match = zoneText.match(/^\s*;\s*agent-shield-content-hash:\s*(\S+)\s*$/m);
  return match?.[1] ?? null;
}
...
function renderZone(opts: WideAreaGatewayZoneOpts & { serial: number }): string {
  const hostname = os.hostname().split(".")[0] ?? "agent-shield";
  const hostLabel = dnsLabel(opts.hostLabel ?? hostname, "agent-shield");
  const instanceLabel = dnsLabel(opts.instanceLabel ?? `${hostname}-gateway`, "agent-shield-gw");
  ...
  records.push(`_agent-shield-gw._tcp IN PTR ${instanceLabel}._agent-shield-gw._tcp`);
  records.push(
    `${instanceLabel}._agent-shield-gw._tcp IN SRV 0 0 ${opts.gatewayPort} ${hostLabel}`,
  );
  records.push(`${instanceLabel}._agent-shield-gw._tcp IN TXT ${txt.map(txtQuote).join(" ")}`);
  ...
  return `; agent-shield-content-hash: ${contentHash}\n${contentBody}`;
}
