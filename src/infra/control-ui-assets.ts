...
export async function resolveControlUiDistIndexPath(
  ...
  const packageRoot = await resolveAgentShieldPackageRoot({ argv1: normalized, moduleUrl });
  if (packageRoot) {
    return path.join(packageRoot, "dist", "control-ui", "index.html");
  }

  // Fallback: traverse up and find package.json with name "agent-shield" + dist/control-ui/index.html
...
export function resolveControlUiRootSync(opts: ControlUiRootResolveOptions = {}): string | null {
  ...
  if (argv1Dir) {
    // agent-shield.mjs or dist/<bundle>.js
