...
function summarizeGroupPolicy(cfg: AgentShieldConfig): {
  open: number;
  allowlist: number;
  other: number;
} {
...
function isGatewayRemotelyExposed(cfg: AgentShieldConfig): boolean {
...
function collectModels(cfg: AgentShieldConfig): ModelRef[] {
  const out: ModelRef[] = [];
...
function listKnownNodeCommands(cfg: AgentShieldConfig): Set<string> {
  const baseCfg: AgentShieldConfig = {
    ...cfg,
    gateway: {
      ...cfg.gateway,
      nodes: {
...
function hasWebSearchKey(cfg: AgentShieldConfig, env: NodeJS.ProcessEnv): boolean {
...
function isWebSearchEnabled(cfg: AgentShieldConfig, env: NodeJS.ProcessEnv): boolean {
...
function isWebFetchEnabled(cfg: AgentShieldConfig): boolean {
...
function isBrowserEnabled(cfg: AgentShieldConfig): boolean {
  try {
...
function listGroupPolicyOpen(cfg: AgentShieldConfig): string[] {
  const out: string[] = [];
  const channels = cfg.channels as Record<string, unknown> | undefined;
  if (!channels || typeof channels !== "object") {
    return out;
...
export function collectSyncedFolderFindings(params: {
  ...
  if (isProbablySyncedPath(params.stateDir) || isProbablySyncedPath(params.configPath)) {
    findings.push({
      ...
      remediation: `Keep AGENT_SHIELD_STATE_DIR on a local-only volume and re-run "${formatCliCommand("agent-shield security audit --fix")}".`,
    });
  }
  return findings;
}

export function collectSecretsInConfigFindings(cfg: AgentShieldConfig): SecurityAuditFinding[] {
  const findings: SecurityAuditFinding[] = [];
  const password =
  ...
  if (password && !looksLikeEnvRef(password)) {
    findings.push({
      ...
      remediation:
        "Prefer AGENT_SHIELD_GATEWAY_PASSWORD (env) and remove gateway.auth.password from disk.",
    });
  }
...
export function collectHooksHardeningFindings(
  ...
  const agentShieldGatewayToken =
    typeof env.AGENT_SHIELD_GATEWAY_TOKEN === "string" && env.AGENT_SHIELD_GATEWAY_TOKEN.trim()
...
export function collectGatewayHttpSessionKeyOverrideFindings(
  ...
  findings.push({
    ...
    detail:
      `${enabledEndpoints.join(", ")} accept x-agent-shield-session-key for per-request session routing. ` +
...
export function collectSandboxDockerNoopFindings(cfg: AgentShieldConfig): SecurityAuditFinding[] {
  const findings: SecurityAuditFinding[] = [];
...
export function collectModelHygieneFindings(cfg: AgentShieldConfig): SecurityAuditFinding[] {
  const findings: SecurityAuditFinding[] = [];
...
export function collectExposureMatrixFindings(cfg: AgentShieldConfig): SecurityAuditFinding[] {
  const findings: SecurityAuditFinding[] = [];
