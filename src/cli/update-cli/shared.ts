...
export function resolveGitInstallDir(): string {
  const override = process.env.AGENT_SHIELD_GIT_DIR?.trim();
  if (override) {
...
export async function resolveUpdateRoot(): Promise<string> {
  return (
    (await resolveAgentShieldPackageRoot({
...
export async function ensureGitCheckout(params: {
  ...
  if (!dirExists) {
    return await runUpdateStep({
      name: "git clone",
      argv: ["git", "clone", AGENT_SHIELD_REPO_URL, params.dir],
      timeoutMs: params.timeoutMs,
  ...
  if (!(await isCorePackage(params.dir))) {
    throw new Error(`AGENT_SHIELD_GIT_DIR does not look like a core checkout: ${params.dir}.`);
  }
