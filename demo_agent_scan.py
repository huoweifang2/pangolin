...
async def demo_agent_scan():
    ...
    for name, result in scan_results.items():
        ...
        # Labels
        labels = []
        if result.labels.destructive > 0:
            labels.append(f"Destructive({result.labels.destructive})")
        if result.labels.is_public_sink > 0:
            labels.append(f"PublicSink({result.labels.is_public_sink})")
        if result.labels.private_data > 0:
            labels.append(f"PrivateData({result.labels.private_data})")
