...
type EnvSnapshot = {
  ...
  agentShieldHome: string | undefined;
...
function snapshotEnv(): EnvSnapshot {
  return {
    ...
    agentShieldHome: process.env.AGENT_SHIELD_HOME,
    stateDir: process.env.AGENT_SHIELD_STATE_DIR,
  };
}
...
function restoreEnv(snapshot: EnvSnapshot) {
  ...
  restoreKey("AGENT_SHIELD_HOME", snapshot.agentShieldHome);
  restoreKey("AGENT_SHIELD_STATE_DIR", snapshot.stateDir);
}
...
function setTempHome(base: string) {
  ...
  // Ensure tests using HOME isolation aren't affected by leaked AGENT_SHIELD_HOME.
  delete process.env.AGENT_SHIELD_HOME;
  process.env.AGENT_SHIELD_STATE_DIR = path.join(base, ".agent-shield");
...
export async function withTempHome<T>(
  ...
  const base = await fs.mkdtemp(path.join(os.tmpdir(), opts.prefix ?? "agent-shield-test-home-"));
  ...
  setTempHome(base);
  await fs.mkdir(path.join(base, ".agent-shield", "agents", "main", "sessions"), {
    recursive: true,
  });
  if (opts.env) {
